...
export default function AdminDashboard() {
  ...
  const handleDeleteEvent = async (eventId: string) => {
    ...
    try {
      const res = await fetch('/api/admin/batch-events', {
        method: 'DELETE',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ eventId, requesterEmail: user?.email }),
      });
      const data = await res.json();
      if (data.success) {
  ...
  const handleRemoveStudent = async (studentEmail: string) => {
    if (!confirm(`Remove ${studentEmail} from their batch?`)) return;
    ...
    try {
      ...
      const data = await res.json();
      if (data.success) {
        showMessage(`${studentEmail} removed from batch!`, 'success');
        fetchBatches();
        fetchStudents();
      } else {
      ...
      showMessage('Network error removing student', 'error');
    } finally {
                ...
                <div key={batch.id} style={{ background: 'rgba(255,255,255,0.03)', border: '0.5px solid rgba(255,255,255,0.06)', borderRadius: 10, padding: 16 }}>
                  ...
                  {batch.users?.length > 0 && (
                    <div style={{ marginBottom: 8 }}>
                      <div style={{ fontSize: 10, color: '#4A5568', marginBottom: 6 }}>STUDENTS</div>
                      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                        {batch.users.map((u: any) => (
                          <div key={u.id} style={{ display: 'flex', alignItems: 'center', gap: 6, background: 'rgba(255,255,255,0.04)', borderRadius: 6, padding: '3px 8px' }}>
                            <span style={{ fontSize: 11, color: '#8899BB' }}>{u.email}</span>
                            <button
                              onClick={() => handleRemoveStudent(u.email)}
                              disabled={removingStudent === u.email}
                              style={{ background: 'none', border: 'none', color: '#F87171', cursor: 'pointer', fontSize: 12, padding: '0 2px', lineHeight: 1 }}
                              title={`Remove ${u.email} from batch`}
                            >
                              {removingStudent === u.email ? '...' : '✕'}
                            </button>
                          </div>
                        ))}
                      </div>
